...
Four experiments, each in its own directory, written to teach one part of
yuclid at a time. Every directory holds a `yuclid.json` and the workload it
measures.
...
| [mandelbrot](mandelbrot/) | Teach points that exist only under a condition, a name that differs from the value, and a dimension chosen at runtime rather than compiled in |
...
The first two need only a C compiler and Python. `mandelbrot` needs two
compilers that understand `-fopenmp`. `pointer-structures` measures hardware
counters, so it needs Linux with `perf` and `strace`.
...
`-r 5` repeats each point five times, and `--compile run.sh` writes the
experiment out as a plain shell script.
