...
- add: panel to observe the real-time progress of the command with options to enable stdout, stderr, or both
- add: real-time plotter inside web UI with x/y/z parameters composable via buttons
- add: workspace options accessible in the UI including: update frequency, enable/disable the creation of JSONL files in the CWD, preferred log order, preferred space nesting order
